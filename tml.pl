...
@synonym=[];        #array of alias definitions
$aliasesCompiled ="no";#Flag to identify if aliases have been loaded into array.
...
#If the option is escaped by '\{' don't process.
if (/\\{.+}/){}
#Otherwise process
else {
    replaceConfigBlocks();
...
#If the tag is escaped by '\[' don't process.
if (/\\\[.+\]/){}
...
sub replaceMetadata {
    #If the sequence is not escaped with a backslash '\'
    if (/\\{\s*catalogue(.+)\s*}/) {}
    else {
		$_ =~ s/\{catalogue-author:\s*(.+)\}/\.AUTHOR $1/;
		$_ =~ s/\{catalogue-editor:\s*(.+)\}/\.EDITOR $1/;
		$_ =~ s/\{catalogue-subtitle:\s*(.+)\}/\.SUBTITLE "$1"/;
		$_ =~ s/\{catalogue-draft:\s*(.+)\}/\.DRAFT $1/;
		$_ =~ s/\{catalogue-revision:\s*(.+)\}/\.REVISION $1/;
		$_ =~ s/\{catalogue-pdf-title:\s*(.+)\}/\.PDF_TITLE "$1"/;
		#If More than one title: Title1 / Title 2
		if (/\{catalogue-title:\s*(.+)\s*\/\s*(.+)\}/){
            $_ =~ s/\{catalogue-title:\s*(.+)\s*\/\s*(.+)\}/\.TITLE "$1" "$2"/;
        #If Only one title: Title
        } elsif (/\{catalogue-title:\s*(.+)\}/){
            $_ =~ s/\{catalogue-title:\s*(.+)\}/\.TITLE "$1"/;
		}
	}
}
...
sub replaceFootnote(){
		if ($_ =~ m/[^\\]\[\*\s*(.+)[^\\]\]/){
			$_ =~ s/\[\*\s*(.+)\]/<footnote<$1>/g; # Replace [*...] with <footnote<...> 
...
sub replaceEndnote(){
		if ($_ =~ m/\[\+\s*(.+)\]/){
            $_ =~ s/\[\+\s*(.+)\]/<endnote<$1>/g; # Replace [*...] with <footnote<...> 
            ...
            if ($tmlfile[$i] eq "\n" ) {
                ...
                if ($tmlfile[$i] =~ /\s*(.+):\s*(.+)/) {
                    if ( grep $_ eq $block, @blocktypes ) {
                        ...
                        if ($block eq "blockquotes") { $block = "blockquote";}
                        if ($block eq "chapters") { $block = "chapter";}
                        if ($block eq "margins") { $block = "margin";}
                        $tmlfile[$i] = "{$block-$1: $2}\n";
                    } else {$tmlfile[$i] = "{$1: $2}\n";}
                }
             }
...
sub replaceCommentBlock{
        if ($_ =~ s/\[comment\]/\.COMMENT/){
           push(@elementStack, ".COMMENT OFF");
        }
}
sub replaceEscapes {
    $re_left_curly = "\\{";
    if (/$re_left_curly/) { $_ =~ s/$re_left_curly/{/g;}
    if (/\\}/) { $_ =~ s/\\}/}/g;}
    #If this is a groff '\[xx]' command, don't escape the brackets
    if (/\\\[.+[^\\]\]/){}
    #Otherwise escape '\['
    else {
        if (/\\\[/) { $_ =~ s/\\\[/[/g;}
...
sub replaceAliases {
    $j=0;
    #If we have not loaded the aliases yet, do this
    if ($aliasesCompiled eq "no") {
        if (/\.\\# ALIASES/){
            for ($i=$currentElement+2;$i<100;$i++){
                if ($tmlfile[$i] =~ /(.+[^\s])\s*=\s*(.+)/){
                    $alias[$j]= trim($1);
                    $synonym[$j]= trim($2);
                    $tmlfile[$i] = ".\\# $1 = $2\n";
    ...
    #Otherwise don't do this previous "if", just skip to this one and replace any alias on this line with its definition.
    }else {
        #If any of the aliases are found within {..}, [...] or <...<, replace them with their synonym.
        foreach ($i=0; $i<$#alias+1;$i++) {
            $re_alias = "([{\\[<].*?)" . $alias[$i] . "(.*?[\\]}<])";
            if (/$re_alias/) {
                $re_synonym = $1 . $synonym[$i] . $2;
                $_ =~ s/$re_alias/$re_synonym/g;
            }
        }
    }
}
