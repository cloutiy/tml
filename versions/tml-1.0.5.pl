...
#{item-spacing: 0.2cm} => sets the space between list items.
#{setup-block}...{end} => for setting blocks of directives
...
sub replaceAlign{
    ...
    if (/{align-right}/){
        $_ =~ s/{align-right}/\.RIGHT/;
    }
    if (/{align-center}/){
        $_ =~ s/{align-center}/\.CENTER/;
    }
}
sub replaceMetadata {
		$_ =~ s/\[author\](.*?)/\.AUTHOR $1/;
...
sub replaceConfigBlocks{
    # If setup-aliases, lets skip it, we have a function specially for that
    if (/{setup-aliases}/) { 
        next;
    } else {
        #If it's a setup-block, lets go through it until {end} and add some braces to the naked commands
        if (/{setup-(.+)}/) {
            #Dont forget to turn this line into a comment
            $block = $1;
            $_ =~ s/{setup-(.+)}/\.\\" setup-$1/;
            #Go through each line until {end} and put the braces around the directives
            for ($i=$currentElement+1; $i < 100; $i++) {
                if ($tmlfile[$i] =~ /{end}/) {
                    $tmlfile[$i] = ".\\\"" . "end setup-$block\n";
                    last;
                } else {
                    if ($tmlfile[$i] =~ /\s*(.+):\s*(.+)/) {
                        $tmlfile[$i] = "{$1: $2}\n";
